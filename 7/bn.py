...
class Variable(object):
    ...
    def calculate_marginal_probability(self):
        ...
        if len(self.parents) == 0:
            self.marginal_probabilities = self.probability_table[list(
                self.probability_table.keys())[0]]
        else:
            ...
            for key, v in self.probability_table.items():
                ...
                parents_probability = multiply_vector_elements(
                    parents_probability_array)
...
def sprinkler():
    ...
    # the values always sums to 1.. the last is the properbility eg ((rest to 1) .7, (properbitiy) .3),
    t5 = {
        ('true', 'true'): (.95, .05),
        ('true', 'false'): (.4, .6),
        ('false', 'true'): (.7, .3),
    ...
    t6 = {
        ...
        ('false', 'false', 'true'): (.9, .1),
        ('false', 'false', 'false'): (.99, .01)
    }

    # creation of Nodes objects
    tireDamage = Variable('DT',         ('false', 'true'), t1)
    electricMalfunction = Variable('EM', ('false', 'true'), t2)
    leak = Variable('FTL',              ('false', 'true'), t3)
    vibrations = Variable('V',          ('false', 'true'), t4, [tireDamage])
    slowMaxSpeed = Variable('SMS',      ('false', 'true'), t5, [
                            tireDamage, electricMalfunction])
    highComsumption = Variable('HC',    ('false', 'true'), t6, [
                               tireDamage, electricMalfunction, leak])

    variables = [tireDamage, electricMalfunction, leak,
                 vibrations,  slowMaxSpeed, highComsumption]
    ...
    conditionals_evidents = {'V': 'true', 'SMS': 'true', 'HC': 'false'}

    print_conditional_probability(
        network, conditionals_vars, conditionals_evidents)
